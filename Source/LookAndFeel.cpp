...
// setup font
const juce::Typeface::Ptr Fonts::typeface = juce::Typeface::createSystemTypefaceFor(BinaryData::JosefinSansRegular_ttf, BinaryData::JosefinSansRegular_ttfSize);
const juce::Typeface::Ptr Fonts::headerTypeface = juce::Typeface::createSystemTypefaceFor(BinaryData::BodoniModa_28ptExtraBoldItalic_ttf, BinaryData::BodoniModa_28ptExtraBoldItalic_ttfSize);
...
juce::Font Fonts::getHeaderFont(float height)
{
    return juce::Font(headerTypeface).withHeight(height);
}
...
void RotaryKnobLookAndFeel::drawRotarySlider(juce::Graphics &g, int x, int y, int width, [[maybe_unused]] int height, float sliderPos, float rotaryStartAngle, float rotaryEndAngle, juce::Slider &slider)
{
    ...
    auto knobRect = bounds.reduced(10.0f, 10.0f); // adds a 10px margin around the knob
    ...
    auto innerRect = knobRect.reduced(MARGIN, MARGIN);
    ...
    auto radius = bounds.getWidth() / MARGIN;
    auto lineWidth = 3.0f;
    auto arcRadius = radius - lineWidth/MARGIN;
    ...
    // draw the dial
    auto dialRadius = innerRect.getHeight() / MARGIN - lineWidth;
    auto toAngle = rotaryStartAngle + sliderPos * (rotaryEndAngle - rotaryStartAngle);
    juce::Point<float> dialStart(center.x + 5.0f * std::sin(toAngle), center.y - 5.0f * std::cos(toAngle));
